...
import com.fafa.common.response.ApiResponse;
...
public class UserTagController {
    ...
    @PostMapping
    public ApiResponse<Long> createTag(@Validated @RequestBody CreateUserTagRequest request) {
        ...
        return ApiResponse.success(userTag.getId().getValue());
    }

    @Operation(summary = "查询用户所有标签")
    @GetMapping
    public ApiResponse<List<UserTag>> getUserTags() {
        Long userId = StpUtil.getLoginIdAsLong();
    ...
    @GetMapping("/popular")
    public ApiResponse<List<UserTag>> getPopularTags(@RequestParam(defaultValue = "20") Integer limit) {
        Long userId = StpUtil.getLoginIdAsLong();
    ...
    @DeleteMapping("/batch")
    public ApiResponse<Void> batchDeleteTags(@RequestBody List<Long> tagIds) {
    ...
    @GetMapping("/count")
    public ApiResponse<Integer> countUserTags() {
        Long userId = StpUtil.getLoginIdAsLong();
